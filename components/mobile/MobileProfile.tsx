      ...
      <div className="fixed inset-0 z-[100] bg-[#0B0C15] overflow-y-auto">
        <div className="sticky top-0 z-[110] p-4 bg-[#0B0C15]/90 backdrop-blur-md border-b border-white/5 flex items-center gap-3">
          <button
            ...
            className="bg-white/5 rounded-full p-2 hover:bg-white/10"
          >
      ...
      <div className="fixed inset-0 z-[100] bg-[#0B0C15] flex flex-col font-sans">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white focus:border-[#f5c619] focus:outline-none"
        ...
        <div className="p-6 pb-12">
          <button
            ...
            disabled={isSaving}
            className="w-full bg-[#f5c619] text-[#0B0C15] font-bold py-4 rounded-full flex items-center justify-center gap-2 disabled:opacity-50"
          >
  ...
  return (
    <div className="bg-[#0B0C15] font-sans text-slate-400 min-h-screen overflow-x-hidden w-full pb-24">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 bg-[#0B0C15]/80 backdrop-blur-md border-b border-white/5">
        ...
        <button
          className="text-white/70 p-2 -mr-2 rounded-full hover:bg-white/5"
      ...
      <main className="flex flex-col items-center px-5 pt-6 pb-12 w-full gap-8 max-w-[480px] mx-auto">
        ...
        <div className="w-full space-y-4">
          <div className="flex items-center justify-between px-1">
            ...
            <button
              ...
              className="text-xs font-bold text-[#f5c619] uppercase"
            >
              View All
            </button>
          </div>
          {myBookings.length === 0 ? (
            ...
            <div className="space-y-4">
              {myBookings.slice(0, 2).map((booking) => (
                <div
                  key={booking.id}
                  className="flex flex-col gap-4 p-4 rounded-[1.5rem] bg-[#13151f] border border-white/5"
                  onClick={() => router.push(`/book/${booking.id}`)}
                >
                  <div className="flex gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="inline-block mt-2 px-2.5 py-1 rounded-full text-xs font-semibold border border-white/10 text-slate-300">
                        {booking.status}
                      </span>
                    </div>
                    <span className="font-bold text-[#f5c619]">
                      {booking.services?.price} ETB
                    </span>
                  </div>
                  {booking.status === 'paid' && (
                    <button
            ...
            <div className="flex gap-4 w-full">
              <div className="flex-1 p-4 rounded-[1.5rem] bg-blue-500/10 border border-blue-500/20">
                ...
                <p className="text-xl font-bold text-white">
                  {(stats?.escrow_balance || 0).toLocaleString()}{' '}
                ...
                {pendingRequests.map((req: any) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm font-medium text-[#f5c619] mt-2">
                        Potential Earnings: {req.services?.price} ETB
                      </p>
                    </div>
                    <div className="flex gap-3">
                      <button
                        ...
                        className="flex-1 py-2.5 rounded-full bg-red-500/10 text-red-400 border border-red-500/20 font-semibold text-sm flex justify-center items-center gap-1"
                      >
                        {isUpdating === req.id ? (
                      ...
                      <button
                        ...
                        disabled={isUpdating === req.id}
                        className="flex-1 py-2.5 rounded-full bg-green-500/10 text-green-400 border border-green-500/20 font-semibold text-sm flex justify-center items-center gap-1"
                      >
                        {isUpdating === req.id ? (
                          <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
        ...
        <div className="w-full mt-4 pt-4">
          <button
            ...
            className="w-full bg-[#f5c619] disabled:opacity-70 text-[#0B0C15] font-bold text-base py-4 px-6 rounded-full flex items-center justify-center gap-2"
          >
            {isLoggingOut ? (
              <Loader2 className="w-5 h-5 animate-spin" />
            ) : (
